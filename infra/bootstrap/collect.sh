...
if [ -s "$OUT/hcloud_token" ]; then ok "already collected"; else
  ...
  read -rsp "   paste token (hidden): " HC
  echo
  printf '%s' "$HC" >"$OUT/hcloud_token"
  chmod 600 "$OUT/hcloud_token"
  ok "saved"
fi
...
if [ -f "$AJ" ]; then
  cp "$AJ" "$OUT/opencode-auth.json"
  chmod 600 "$OUT/opencode-auth.json"
  ok "copied auth.json"
...
if [ -s "$OUT/objectstorage" ]; then ok "already collected"; else
  ...
  if [[ "${YN:-}" =~ ^[Yy] ]]; then
    ...
    read -rsp "   S3 secret key (hidden): " SK
    echo
    read -rp "   bucket [tidepool-tfstate]: " BK
    BK="${BK:-tidepool-tfstate}"
...
gen_age() { # $1 = name
  ...
  chmod 600 "$OUT/age-$1.key"
  ok "age-$1 generated"
}
gen_age mainbox
gen_age breakglass
if [ -f "$OUT/ssh-tidepool" ]; then ok "ssh key exists"; else
  ssh-keygen -t ed25519 -N "" -C "tidepool" -f "$OUT/ssh-tidepool" >/dev/null
