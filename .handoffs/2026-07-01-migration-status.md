...
- **PR-6.5 (tckt_cp5dep)** `tp/tckt_cp5dep-controlplane-deploy` — the **control-plane k8s Deployment** = the LIVE flip (workloads.ts still only had ns+RBAC; control-plane ran on the box). Adds: `k8s.apps.v1.Deployment` (ns `tidepool`, SA `tidepool-control-plane`, **replicas:1 + Recreate** singleton) via a pure @pulumi-free spec builder `control-plane-deployment.ts` (+ red-first test); env gate `TIDEPOOL_DB_DRIVER=pg` / `TIDEPOOL_AGENT_WORKER=k8s`; `TIDEPOOL_PG_URL`←CNPG `tidepool-pg-app`/`uri`; `GITHUB_TOKEN`←new k8s Secret `tidepool-control-plane-secrets`; opencode blob projected at `/root/.tidepool/bootstrap/opencode-auth.json`; images **digest-pinned** via config `controlPlaneImage`/`agentWorkerImage`. Widens `ci` to 5 secrets (.sops.yaml + infra.yml up-job decrypt). **Reseal of the 2 newly-ci-readable .enc.yaml files pending (breakglass, main session).** Data-move is a **no-op** (no live sqlite data) → acceptance = one fresh ticket on pg.
- **PR-7** pending (blocked on PR-6).

## Secrets matrix (least-priv — verified)
`SOPS_AGE_KEY` (=`ci` age PRIVATE key) is the **ONLY GH Actions secret**. `ci` decrypts **5 secrets** (was 3;
widened in **PR-6.5**, human-approved tenet-9): `hcloud_api_token`, `hetzner_s3_credentials`,
`pulumi_config_passphrase`, **+ `forge_github_token` + `runner_opencode_auth_json`** (so `pulumi up` can seed
the control-plane k8s Secret). NOT `ssh_tidepool_private_key` (mainbox+breakglass only). breakglass = every
secret (recovery). ⚠️ the ci widening MUST be re-scoped/rotated in the deferred rotation pass.
...
- **Rotate EVERYTHING** (breakglass leaked → all 3 age keys + every secret value) — handover `.handoffs/INVESTIGATE-secret-leak-guard.md`. **Also re-scope the PR-6.5 `ci` widening** (ci now decrypts `forge_github_token` + `runner_opencode_auth_json`) — narrow back if a tighter delivery lands, else rotate those two values. Parallel worker (branch `tp/tckt_lk9g2r`) landed the LIVE `secret-command-guard` (PreToolUse block) + `secret-redactor` — confirmed working (blocks even the orchestrator from key material). KEEP rotation deferred until migration done.
