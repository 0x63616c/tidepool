...
The control plane runs as a Kubernetes Deployment (on a Talos/Hetzner cluster) with a reconciler loop
+ a Postgres ticket store (CloudNativePG, the single source of truth). When tickets exist it dispatches
ephemeral k8s Jobs that drive coding agents (via the opencode SDK, on your Codex subscription) against a
...
infra/          Pulumi k8s cluster (Talos, CNPG, CCM/CSI) + Docker images + bootstrap
