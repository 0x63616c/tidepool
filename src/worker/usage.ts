...
/**
 * Typed rollup of an opencode event stream into one `Usage`. This is the single
 * source for usage parsing: the in-process runner and the bun-built remote
 * runner both import it, so the parser can never drift between the two ends of
 * the seam (the old stringified-JS runner kept a hand-copied duplicate).
 */
