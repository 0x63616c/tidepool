...
**Leak guards** (`.claude/hooks/`, bash 3.2-safe): PreToolUse `secret-command-guard` blocks/asks on commands that would print a secret; PostToolUse `secret-redactor` masks secret shapes + our exact sealed values (`.claude/redaction-hashes.json`, regenerated by `bun run seal:hashes` on any secret change, entropy-gated ≥80 bits, drift-checked at pre-push). Age keys are X25519 (not post-quantum) — see `DESIGN.md`.

**Adding/rotating a secret?** Run `bun run seal:hashes`. If it introduces a new secret *shape* or env-var name, add it to `secret-command-guard.sh` + `secret-redactor.sh` and cover it in `src/secret-hooks.test.ts`.
